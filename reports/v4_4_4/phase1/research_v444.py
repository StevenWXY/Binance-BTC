...
def main():
    ...
    if args.phase == "screen":
        ...
        rows = []
        for p in top:
            legacy = json.loads((OUT / f"{p['id']}__legacy__minute/metrics.json").read_text())
            recent = json.loads((OUT / f"{p['id']}__recent__minute/metrics.json").read_text())
            row = {"id": p["id"], "params": p["params"], "legacy": legacy, "recent": recent,
                   "folds": [recent]}
            row["score"] = score(row)
            row["eligible"] = (legacy["max_drawdown"] >= -.55 and recent["max_drawdown"] >= -.35
                                and recent["cycles"] >= 30)
            rows.append(row)
        chosen = max([r for r in rows if r["eligible"]], key=lambda r: r["score"])
        config_data = {"version": "V4.4.4", "id": chosen["id"], "params": chosen["params"], "fee": REBATE}
        Path("configs/v4_4_4_params.json").write_text(json.dumps(config_data, indent=2) + "\n")
        files = ["src/btc_regime/v444.py", "src/btc_regime/v443.py", "src/btc_regime/v442.py",
                 "src/btc_regime/v441_r2.py", "src/btc_regime/micro_backtest.py", "scripts/research_v444.py",
                 "reports/v4_4_4/protocol.json", "reports/v4_4_4/screen.json", "reports/v4_4_4/minute_finalists.json",
                 "data/v441/hourly.pkl", "data/v441/funding.pkl"]
        lock = {"version": "V4.4.4", "selected": chosen, "frozen_at_utc": datetime.now(timezone.utc).isoformat(),
                "full_history_used_for_screen": True,
